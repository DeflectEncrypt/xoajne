...
-- CONTEXT HELPERS
-- ============================================================
local SKIP_CLASSES = {Sound=true,AudioPlayer=true,AudioEmitter=true,AudioListener=true,AudioAnalyzer=true,AudioDeviceInput=true,AudioDeviceOutput=true,AudioEqualizer=true,AudioCompressor=true,AudioReverb=true,AudioPitchShifter=true,AudioFader=true,Wire=true}
local function getDexContext()
	...
	local function scan(inst, depth)
		...
		for _, c in ipairs(children) do
			...
			if nameOk and classOk and not SKIP_CLASSES[cclass] then
...
-- GUI
-- ============================================================
pcall(function() local old = gui:FindFirstChild("AIAssistGui"); if old then old:Destroy() end end)
